...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, onSelectTab, onOpenLiteModeModal }) => {
  const { currentAdmin, isAuthenticated } = useAdminAuth();
  const { isLiteMode, toggleLiteMode, estimatedDataSavedBytes } = useLiteMode();
    ...
    <header className="sticky top-0 z-40 bg-white/98 backdrop-blur-md border-b border-slate-200 shadow-xs">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-2">
          ...
          {/* CTA Right Actions (Desktop & Mobile) */}
          <div className="flex items-center gap-1.5 sm:gap-2">
            {/* Lite Mode Toggle Pill */}
            <button
              type="button"
              onClick={onOpenLiteModeModal || toggleLiteMode}
              className={`hidden sm:inline-flex items-center gap-1 px-2.5 py-2 rounded-xl text-xs font-semibold border transition-all ${
                isLiteMode
            ...
            >
              <Zap className={`w-3.5 h-3.5 ${isLiteMode ? 'fill-amber-500 text-amber-500' : 'text-slate-400'}`} />
              <span className="hidden xl:inline">{isLiteMode ? 'Hemat Kuota (ON)' : 'Hemat Kuota'}</span>
            </button>

            {/* Layanan Surat CTA */}
            <button
              onClick={() => handleNav('layanan')}
              className={`inline-flex items-center gap-1.5 px-3 sm:px-3.5 py-2 sm:py-2.5 rounded-xl font-bold text-xs shadow-sm transition-all active:scale-95 ${
                isTabActive('layanan')
            ...
            {/* Admin CMS Direct Access Button on Desktop */}
            <button
              onClick={() => handleNav('admin')}
              className={`hidden md:inline-flex items-center gap-1.5 px-3 py-2 sm:py-2.5 rounded-xl font-bold text-xs transition-all ${
                isTabActive('admin')
                  ? 'bg-slate-900 text-emerald-300 ring-2 ring-emerald-500'
                  : isAuthenticated
                  ? 'bg-slate-900 text-emerald-300 hover:bg-slate-800 border border-slate-700 shadow-xs'
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-200 px-4 pt-2 pb-6 space-y-2 shadow-2xl max-h-[82vh] overflow-y-auto animate-in slide-in-from-top-2 duration-200">
          ...
          {/* Lite Mode Banner / Toggle in Mobile Menu */}
          <div className="p-3 rounded-xl bg-slate-900 text-white flex items-center justify-between gap-3 shadow-sm border border-slate-800">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                isLiteMode ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-400'
              }`}>
                <Zap className={`w-4 h-4 ${isLiteMode ? 'fill-current' : ''}`} />
              </div>
              <div className="min-w-0">
                <p className="text-xs font-bold truncate">Mode Hemat Kuota</p>
                <p className="text-[10px] text-slate-400 truncate font-mono">
                  {isLiteMode ? `Aktif (${formatBytes(estimatedDataSavedBytes)} hemat)` : 'Nonaktif (Standar HD)'}
                </p>
              </div>
            </div>
            ...
            <button
              ...
              onClick={() => {
                setMobileMenuOpen(false);
                if (onOpenLiteModeModal) onOpenLiteModeModal();
                else toggleLiteMode();
              }}
